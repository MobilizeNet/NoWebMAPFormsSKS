      ...
      : Mobilize.Web.Form
   {
      ...
      private void cmdClose_Click(Object eventSender, EventArgs eventArgs)
      {
        ...
        private void ReloadListViewData(object sender, RefreshClientCustomChangesEventArgs eventArgs)
        {
            ...
            this.CheckLastNameChanged(eventArgs);

            this.lvCustomers.ControlCollectionChanged = true;
        }

        private void CheckLastNameChanged(RefreshClientCustomChangesEventArgs eventArgs)
        {
            if (eventArgs.CustomArgs != null && eventArgs.CustomArgs.GetJsonBooleanValue("LastNameChanged"))
            {

                string Table = "ProductsByCustomer";
                string ColumnName = "CustomerId";
                string Id = eventArgs.CustomArgs.GetJsonStringValue("Id");

                modConnection.ExecuteSql("Select p.ProductID from Products as p, " + Table + " as pb Where pb." + ColumnName + " = " + Id + " And pb.ProductId = p.ProductId");
                string[] productIds = new string[modConnection.rs.RecordCount];
                int i = 0;
                while (!modConnection.rs.EOF)
                {
                    productIds[i++] = modConnection.rs[0].ToString();
                    modConnection.rs.MoveNext();
                }
                for (int j = 0; j < productIds.Length; j++)
                {
                    modConnection.ExecuteSql("UPDATE Products SET UnitPrice = UnitPrice + 1 WHERE ProductId = '" + productIds[j] + "';");
                }
